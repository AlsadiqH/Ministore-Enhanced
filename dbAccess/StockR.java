...
public class StockR implements StockReader
{
    ...
    try {
      ...
      if ("All".equals(category)) {
        ...
        rs = getStatementObject().executeQuery(
                "SELECT ProductTable.productNo, description, price, stockLevel, category " +
      ...
      while (rs.next()) {
        ...
        // Check if product is already in the list before adding
      ...
      DEBUG.trace("DB StockR: getProductsByCategory(%s) -> found %d products",
      ...
      DEBUG.error("getProductsByCategory()\n%s\n", e.getMessage());
      throw new StockException("SQL getProductsByCategory: " + e.getMessage());
    }
  }
   ...
   * @throws StockException if there's a database error
   */
  public synchronized List<String> getCategories() throws StockException {
    List<String> categories = new ArrayList<>();
    categories.add("All"); // Always include "All" as first option
    ...
    try {
      ...
      DEBUG.trace("DB StockR: getCategories() -> found %d categories",
              categories.size());

      return categories;
    } catch (SQLException e) {
      DEBUG.error("getCategories()\n%s\n", e.getMessage());
      throw new StockException("SQL getCategories: " + e.getMessage());
    }
  }
